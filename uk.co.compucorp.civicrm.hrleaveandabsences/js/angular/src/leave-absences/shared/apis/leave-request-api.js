    ...
    return api.extend({
      ...
      calculateBalanceChange: function (params) {
        ...
        if (params && (!params.contact_id || !params.from_date || !params.from_type)) {
          deferred.reject({
            is_error: 1,
            error_message: 'contact_id, from_date and from_type in params are mandatory',
            params: params
          });
        }
      ...
      create: function (params) {
        ...
        if (params && params.to_date && !params.to_type) {
          deferred.reject({
            is_error: 1,
            error_message: 'to_type is mandatory'
          });
        } else if (params && (!params.contact_id || !params.from_date ||
            !params.from_type || !params.status_id)) {

          deferred.reject({
            is_error: 1,
            error_message: 'contact_id, from_date, status_id and from_type params are mandatory'
          });
        }
